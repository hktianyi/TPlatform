        ...
        <div class="col-md-4">
          <div class="portlet light portlet-fit full-height-content full-height-content-scrollable bordered">
            <div class="portlet-title">
              ...
              <div class="actions">
                <a href="javascript:editDictType();" class="btn btn-circle btn-default btn-sm">
                ...
                <ul class="list-group">
                  <c:forEach items="${dictionaryTypes}" var="dictionaryType">
                    <li class="list-group-item"
                        onclick="findDict('${dictionaryType.id}')"> ${dictionaryType.zhName}
                      <span class="badge badge-danger">
                      <a href="javascript:delDictType('${dictionaryType.id}');">
                        <i class="icon-trash"></i></a>
                    </span>
                      <span class="badge badge-warning">
                      <a href="javascript:editDictType('${dictionaryType.id}');">
        ...
        <div class="col-md-8">
          <div class="portlet light portlet-fit full-height-content full-height-content-scrollable bordered">
            <div class="portlet-title">
              ...
              <div class="actions">
                <a href="javascript:editDict();" class="btn btn-circle btn-default btn-sm">
                  <i class="fa fa-plus"></i> 添加 </a>
              </div>
            </div>
            <div class="portlet-body">
...
<script type="text/javascript">
  ...
  function editDict(id) {
    ...
    layer.open({
      ...
      shade: 0.6,
      area: ['500px', '500px'],
      content: _PATH + '/dict/editDict?dicTypeId=' + dicTypeId + (id ? '&id=' + id : '')
    });
  }
  function delDict(id) {
    layer.confirm('确认删除？', {
      ...
      $.ajax(_PATH + '/dict/deleteDict?id=' + id, {
        type: 'DELETE',
  ...
  function editDictType(id) {
    layer.open({
      ...
      shade: 0.6,
      area: ['500px', '500px'],
      content: _PATH + '/dict/editDictType/${menuType}' + (id ? '?id=' + id : '')
    });
  }
  function delDictType(id) {
    layer.confirm('确认删除？', {
      btn: ['删除', '取消'] //按钮
    }, function () {
      $.ajax(_PATH + '/dict/deleteDictType?id=' + id, {
        type: 'DELETE',
        success: function (resp) {
          if (resp.statusCode === 200) {
            layer.msg('删除成功');
  ...
  function findDict(id) {
    $.ajax(_PATH + '/dict/findDict?dicTypeId=' + id, {
      type: 'GET',
      success: function (resp) {
        dicTypeId = id;
        var html = '';
        if (resp.statusCode === 200) {
          ...
          for (var i = 0, len = data.length; i < len; i++) {
            html += '<tr><td>' + data[i].zhName + '</td><td>' + data[i].value + '</td><td>' + data[i].sort + '</td>' +
                '<td><a href="javascript:delDict(\'' + data[i].id + '\')">删除</a></td></tr>';
          }
        }
